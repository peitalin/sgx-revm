...
// pub fn serve(mut conn: TcpStream, key: &mut Arc<Pk>, cert: &mut Arc<List<Certificate>>) -> TlsResult<()> {
//     let mut rng = Rdrand;

//     let mut config = Arc::new(Config::new(Endpoint::Server, Transport::Stream, Preset::Default));
...
pub fn get_key_and_cert() -> (Pk, Box<Certificate>) {
    let mut rng = Rdrand;
